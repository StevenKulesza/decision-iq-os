...
let frame = {
  ...
  "image": {
    ...
    //Find Evergreen frame
    if(selectedTemplate.evergreenFrame){
      for(let i=0; i < selectedTemplate.template.frames.frames.length; i++){
        if(selectedTemplate.evergreenFrame === selectedTemplate.template.frames.frames[i].name){
          evergreenFrame = selectedTemplate.template.frames.frames[i];
          break;
        }
      }
    }
    ...
    //Visualize Product
    if(selectedTemplate.visualizer && selectedTemplate.visualizer.tabKey === "products" && selectedTemplate.template.products.products[selectedTemplate.visualizer.objectKey]){
      product = selectedTemplate.template.products.products[selectedTemplate.visualizer.objectKey];
    }

    //Visualize Frame
    if(selectedTemplate.visualizer && selectedTemplate.visualizer.tabKey === "frames" && selectedTemplate.template.frames.frames[selectedTemplate.visualizer.objectKey]){
      frame = selectedTemplate.template.frames.frames[selectedTemplate.visualizer.objectKey];
    }
    

    let galleryBackground = (frame.name !== evergreenFrame.name && selectedTemplate.visualizer && selectedTemplate.visualizer.tabKey === "frames") ? frame.image["300x600"].value : templateStyles.gallery.background_image_300x600.value;
    let evergreenFrameBackground = evergreenFrame.image["300x600"].value;
    ...
    const headerStyle = css({
      ...
      backgroundImage: templateStyles.header.background_image_300x600.value ? 'url(' + templateStyles.header.background_image_300x600.value + ')' : null,
      backgroundSize: '100% auto',
      backgroundRepeat: 'no-repeat'
    })
    ...
    const footerStyle = css({
      ...
      backgroundImage: templateStyles.footer.background_image_300x600.value ? 'url(' + templateStyles.footer.background_image_300x600.value + ')' : null,
      backgroundSize: 'cover'
    })

    const backgroundStyle = css({
      backgroundColor: templateStyles.background.background.value,
      backgroundImage: templateStyles.background.background_image_300x600.value ? 'url(' + templateStyles.background.background_image_300x600.value + ')' : null,
      backgroundSize: 'cover'
    })
    ...
    const frameImageStyle = css({
      backgroundImage: evergreenFrameBackground ? 'url(' + evergreenFrameBackground + ')' : null
    });

    return (
      <div className={css`${display.banner300x600}` + ' ' + css`${display.bannerGlobalStyles}` + ' b300x600'}>
        <div className={backgroundStyle + ' banner absolute'}>
          <div className="clicktag-general">
            <div className={ headerStyle + " header absolute"}>
              ...
              <div className="text1 absolute contain center-bg"></div>
              <div className="plus-container absolute">
                <div className="circle absolute">
                  <div className="plus-x absolute"></div>
                  <div className="plus-y absolute"></div>
                </div>
              </div>
              <div className={frameImageStyle + " text2 absolute contain"}></div>
            </div>
