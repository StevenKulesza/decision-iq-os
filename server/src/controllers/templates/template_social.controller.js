...
// model
import Template_social from '../../models/template_social.model'


exports.get_all_templates = function(){
    return new Promise(function(resolve, reject){
      Template_social.find()
      .exec()
      .then(templates => {
        resolve({
          "type": "social",
...
exports.create_template = function(data){
  return new Promise(function(resolve, reject){
    Template_social.find({name: data.name})
    .exec()
    .then(templates => {
      if(templates.length >= 1){
        ...
        //create a new user
        const newTemplate = new Template_social({
...
exports.API_delete_template = function(req, res){
  Template_social.remove({ name: req.body.name })
    .exec()
    .then(result => {
      if(result.n >= 1){
        res.status(202).json({
