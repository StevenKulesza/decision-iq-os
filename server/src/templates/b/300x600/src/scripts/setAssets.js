var setAssets = function (paths) {
  ...
  elem('.bg').style.backgroundImage = 'url("' + paths.styles[audienceStyle].background.background_image_300x600.imageID + '")';
  ...
  elem('.gallery').style.backgroundImage = 'url("' + paths.styles[audienceStyle].gallery.background_image_300x600.imageID + '")';
  ...
  elem('.text1').style.backgroundImage = 'url("' + paths.styles[audienceStyle].header.background_image_300x600.imageID + '")';

  // preload images
  preloadCheck(paths.styles[audienceStyle].background.background_image_300x600.imageID, imageArray);
  preloadCheck(paths.styles[audienceStyle].gallery.background_image_300x600.imageID, imageArray);
  preloadCheck(paths.styles[audienceStyle].cta.background_image.imageID, imageArray);
  preloadCheck(paths.styles[audienceStyle].header.background_image_300x600.imageID, imageArray);

  // if frames are true then push its set images/copy and add to preloader
  for (f in paths.frames) {
    if (paths.frames[f].visible === true &&
      paths.frames[f].evergreen === false) {
      loadFrame('.frame-' + paths.frames[f].name, paths.frames[f].image[size].imageID);
    } else if (paths.frames[f].name !== 'geo') {
      elem('.text2').style.backgroundImage = 'url("' + paths.frames[f].image[size].imageID + '")';
    }
  }
